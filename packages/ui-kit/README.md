...
[滚动条样式](styles/scrollbar.css)以主题作用域限定覆盖；后代伪元素选择器的优先级高于宿主裸选择器，未挂到实际滚动元素的选择器不能改变布局宽度。Chromium 的伪元素路径和标准属性路径不能互相替代。[主题验收](../../tests/e2e/notes/theme-scrollbar.test.ts)记录运行时验证，取舍见[决策](../../.agents/notes/implemented/feature/2026-09-11-themed-scrollbar.md)。
