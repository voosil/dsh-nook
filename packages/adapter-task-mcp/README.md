...
工具 `nook_task_list` 返回任务快照；`nook_task_command` 接受 `requestId` 和 `command`，操作结构由[能力 schema](../capability-task/src/index.ts)发布到 MCP。请求使用稳定 UUID，重试复用；版本值从最近快照读取。状态转换以[任务引擎](../feature-tasks/src/engine.ts)为准。
