...
同步升级及开启前的完整快照位于业务数据目录旁的 `<业务目录>.sync-backups`；接收、冲突解决及通用记录替换的恢复记录位于 `<笔记数据库>.backups`，保留原始版本。数据库快照包含同步历史和待发状态，文件对象位于 `<笔记数据库>.files`，随业务目录备份。私有同步凭据不包含在此目录，恢复后重新配置；同步范围及异常恢复语义见[同步契约](../packages/feature-sync/README.md)。
