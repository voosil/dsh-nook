...
| 孟菲斯            | `memphis` | 浅蓝底、嫩黄按钮、浅粉点缀与亮蓝强调色，更大的圆角 |

四套主题覆盖背景、表面、导航、交互状态、文字、语义色、焦点和阴影。共用间距、动效与层级；`--nook-font-editor` 表达阅读字体，`--nook-font-sans` 表达界面字体。主题选择即时生效，保存在浏览器当前源的本地存储，并响应同源标签页的偏好变更；存储不可用时保留会话内选择。外观偏好不参与笔记同步。
...
工作区左下角的“设置”打开全屏弹窗。左栏列出“外观”“数据同步”“应用更新”，右侧展示对应内容；窄屏保留左右结构。外观页提供主题预览卡片。数据同步保留状态与配置 tab，活动面板独立滚动，底部操作固定；关闭设置不停止后台同步状态轮询。配置指南往返保留表单草稿。

笔记工作区在同一 Client 生命周期内持续挂载，切换到 AI 对话或任务页只隐藏工作区，返回时保留选中笔记、项目视图、筛选、分页、滚动位置和编辑器撤销记录，并重新读取列表与当前笔记版本。隐藏期间设置弹窗关闭模态交互，设置页与配置草稿保留在组件状态中。页面刷新、Client 插件重载和应用重启会创建新的页面状态；此状态不写入笔记数据或同步存储。
...
笔记正文的有序、无序和待办列表中，清空中间项后再按退格或向前删除键会移除该空项，光标落在后一项开头，有序列表编号连续递补。该操作可独立撤销。首尾项沿用编辑器的默认退出列表行为；含嵌套列表或其他内容块的项不视为空项。
