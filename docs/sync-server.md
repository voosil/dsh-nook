...
Nook 同步服务器安装助手在支持的 Linux 服务器上配置 Docker、Tailscale 和独立同步存储。Nook → 数据同步 → 配置 → 配置指南提供当前版本的安装命令。服务器无需运行 Nook Host 或模型；同步范围和连接格式见[同步契约](../packages/feature-sync/README.md)。
...
1. 未配置同步时，在“同步信息”点击“去配置”，或在“配置”页打开“配置指南”并点击“让 AI 帮我配置”，在新建对话中发送已填好的请求；也可点击“复制提示词”交给其他 AI。复制内容与内置对话草稿一致，包含同步指南和固定版本安装信息。剪贴板不可用时展开提示词供手动复制。
2. AI 先根据用户需求选择已有服务接入、新部署或故障排查；需求不明确时先简短询问，确定路径后只获取下一步所需信息。连接已有服务从服务器地址或私有连接文件开始。账号登录和系统提权在对应界面完成。
3. AI 按随附的 [nook-sync-deploy 指南](../packages/feature-agent/skills/nook-sync-deploy/SKILL.md) 和可用工具执行；需要用户操作时逐步引导。结果分别报告服务器可用性、目标设备接入和实际同步情况。

入口通过官方会话 API 创建独立对话并填入草稿，用户发送前不触发模型或部署；原有对话草稿保留。`nook_sync_deployment_guide` 提供同版本指南，不要求另行安装 skill。内置工具操作提供它的 Nook Host；其他目标设备通过各自 Nook 配置与验证。账号登录、系统提权及宿主要求的审批仍可能需要用户参与。

“手动配置”提供已有服务接入和新服务器安装命令。用户可自行执行安装，随后在目标 Nook 粘贴连接信息或导入文件，再点击“验证并开启同步”。
