...
1. 在 Nook 复制“家庭服务器安装命令”，通过 SSH 登录服务器并粘贴执行。
2. 按终端提示开始安装，并完成 Tailscale 登录；已有有效登录不重新授权。
3. 复制终端输出的整段 `NOOK-SYNC-1:…` 连接信息，在 Nook 的“粘贴连接信息”中粘贴。
4. 核对服务器地址，点击“验证并开启同步”。每台电脑也需安装 Tailscale 并登录同一网络。
