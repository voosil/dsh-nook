...
import {
  dismissOnboarding,
  verifyThemedScrollbar,
  themedScrollbarColors,
} from '../../scripts/verify/notebook-smoke.mjs'
  ...
  await bootAndVerifyWeb({
    ...
    acceptance: async (url: string) => {
      ...
      try {
        ...
        // Chromium renders the pseudo-element path, so the idle gutter is the
        // themed width and it must differ from the layout the host applies to
        // its own surfaces outside the workspace.
        ...
        for (const [name, theme] of [
          ...
          const colors = await themedScrollbarColors(page)
          seen.add(colors.hover)
          assert.ok(colors.thumb.startsWith('#') || colors.thumb.startsWith('rgb'))
        }
        assert.equal(seen.size, 8, 'Each appearance carries its own scrollbar thumb and hover color')
