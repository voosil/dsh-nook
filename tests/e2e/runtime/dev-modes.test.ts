...
async function stop(child: ChildProcess, processes: ProcessScope) {
  if (child.exitCode !== null || child.signalCode !== null) return
  const exited = once(child, 'exit')
  if (process.platform === 'win32' && child.connected) child.send({ type: 'stop' }, () => {})
  else child.kill('SIGTERM')
  const timer = setTimeout(() => {
    void processes.stop(child).catch(error => console.error(error.message))
  }, 15_000)
  try {
    await exited
  } finally {
...
test(
  'dev reloads Client and Host while start serves its fixed build',
  {
    ...
    skip:
      ...
      'Formal startup requires Windows or macOS arm64',
  },
  async t => {
    ...
    t.after(async () => {
      try {
        await Promise.all(children.map(child => stop(child, processes)))
      } finally {
      ...
      await cp(resolve(ROOT, name), resolve(root, name), {
        ...
        filter: path =>
          !path
            .replaceAll('\\', '/')
            .split('/')
            .some(part => ['node_modules', 'dist', 'resources'].includes(part)) &&
    ...
    const launch = (script: string) => {
      ...
      const child = processes.spawn(
        ...
        [
          ...(process.platform === 'win32'
            ? ['--import', pathToFileURL(resolve(ROOT, 'apps/desktop/dist/windows-shutdown.mjs')).href]
            : []),
        ...
        {
          cwd: root,
          stdio: ['ignore', 'pipe', 'pipe', ...(process.platform === 'win32' ? ['ipc' as const] : [])],
          env: { ...process.env, DSH_HOME: resolve(root, '.dsh-dev') },
        },
      )
      children.push(child)
      if (process.platform === 'win32') child.send({ type: 'start' })
    ...
    await stop(dev.child, processes)
    await stop(stable.child, processes)
