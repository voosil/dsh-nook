...
import { ROOT, run } from '../../../../scripts/profile/profile-lib.mjs'
import { verifyBackupRecovery } from '../../../helpers/scenarios/notebook-outcomes.mjs'
test('packaged desktop preserves data, exports, recovery and shared lifecycle', { timeout: 1200000 }, async t => {
  ...
  if (process.platform !== 'darwin' || process.arch !== 'arm64')
    throw new Error('Run packaged desktop acceptance on macOS arm64')
  ...
  try {
    ...
    await run(
      ...
      [
        resolve(ROOT, 'tests/fixtures/desktop/native.mjs'),
    ...
    assert.deepEqual(
      ...
      'The renderer must not expose Node or arbitrary module loading',
    )
    ...
    const expected = await runNotebookAcceptance(t, {
      ...
      node: join(payload, 'node/bin/node'),
    })
    const { title } = expected
    const exported = (await readdir(join(state, 'exports'))).filter(name => name.endsWith('.md'))
    const contents = await Promise.all(exported.map(name => readFile(join(state, 'exports', name), 'utf8')))
    assert.ok(
      contents.some(text => text.includes(title) && text.includes('右键操作必须保留刚输入的正文')),
      'Native export must contain the authored title and text',
    )
    assert.equal(
    ...
    // Reopen the same installed state and read previously saved user content.
    ...
    await verifyBackupRecovery(state, before, expected, {
      profileDirectory: join(state, 'harness/profiles/nook'),
      node: join(payload, 'node/bin/node'),
    })
    ...
    assert.equal(
      await sharedWorkspace.getByRole('textbox', { name: '笔记标题', exact: true }).inputValue(),
      sharedTitle,
    )
    ...
    for (const version of versions) {
      const license = join(state, 'runtimes', version, 'node', 'LICENSE')
      const content = await readFile(license)
      damaged.push({ license, content })
    ...
    for (const { license, content } of damaged) await writeFile(license, content)
    ...
    console.log(
      `Verified packaged Nook window, renderer Node isolation, authentication, saved content, backup recovery, restart, integrity failure/retry, and cleanup. First start: ${firstStartMs} ms; second start: ${secondStartMs} ms. Screenshot: ${artifact}`,
    )
  } catch (error) {
    try {
