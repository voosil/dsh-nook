...
// A separate process tries the same lock; successful API calls alone do not
// establish that the shipped native module enforces exclusion.
const contender = () =>
  spawnSync(
    process.execPath,
    [
  ...
  const { flockSync } = createRequire(process.argv[1])('fs-ext')
  const fd = openSync(process.argv[2], 'a')
  try { flockSync(fd, 'exnb') }
  catch (error) {
    if (['EAGAIN', 'EWOULDBLOCK'].includes(error.code)) process.exitCode = 73
    else throw error
  } finally { closeSync(fd) }
`,
      base.resolve('@deepseek-ai/dsh-session-persistence-jsonl/package.json'),
      join(scratch, 'native-lock'),
    ],
...
try {
  ...
  assert.equal(blocked.status, 73, blocked.stderr || 'Another process must not acquire an owned lock')
} finally {
  closeSync(fd)
}
const available = contender()
assert.equal(available.status, 0, available.stderr || 'The lock must be usable after its owner closes')
