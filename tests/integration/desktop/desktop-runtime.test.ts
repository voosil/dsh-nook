...
test(
  ...
  async () => {
    ...
    let restarted: DesktopRuntime | undefined
    try {
      ...
      assert.throws(() => acquireDataLock(join(config.home, 'nook')), /locked/)
      const exited = once(child, 'exit')
      child.kill('SIGKILL')
      await exited
      await assertGone(pids)
      acquireDataLock(join(config.home, 'nook'))()
      restarted = new DesktopRuntime(
        config,
        () => {},
        () => {},
      )
