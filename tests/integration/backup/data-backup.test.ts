...
test(
  'POSIX Host retains the data lock after its supervisor releases the inherited descriptor',
  { skip: process.platform === 'win32', timeout: 10_000 },
  async t => {
    const root = await mkdtemp(join(tmpdir(), 'nook-inherited-lock-'))
    t.after(() => rm(root, { recursive: true, force: true }))
    const source = join(root, 'nook')
    const release = acquireDataLock(source)
    t.after(release)
    const worker = join(root, 'host.mjs')
    await writeFile(worker, "process.on('message', () => {}); process.send('ready')")
    const child = fork(worker, { execArgv: [], stdio: ['ignore', 'ignore', 'pipe', release.fd, 'ipc'] })
