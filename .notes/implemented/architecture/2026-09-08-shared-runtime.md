...
创建新后端需要准备或校验运行时快照，加入现有后端只建立连接。当前后端版本和端口优先；[应用更新](../feature/2026-09-10-application-update.md)统一切换后端，手动应用源码、配置或端口变更需要关闭所有正式入口再启动。两个前端的页面状态各自独立，数据共享不等于即时同步尚未保存的编辑。
