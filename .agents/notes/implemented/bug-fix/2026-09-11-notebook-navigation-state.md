...
The [browser regression](../../../../tests/e2e/notes-navigation.test.ts) uses a temporary DSH home with 55 notes. It covers return-button, Escape and task-page navigation, selection, project filtering, search, sorting, second-page retention, editor scrolling, undo history, project view and trash view. The [shared smoke acceptance](../../../../scripts/verify/notebook-smoke.mjs) checks selected-note restoration in linked and packed Profiles and confirms that hidden settings release the AI composer and reopen at the same settings page.
